...
public class MyDefaultProcessDiagramGenerator extends DefaultProcessDiagramGenerator {
    ...
    private static MyDefaultProcessDiagramGenerator myDefaultProcessDiagramGenerator;

    @PostConstruct
    public void init() {
        myDefaultProcessDiagramGenerator = this;
        myDefaultProcessDiagramGenerator.historyService = this.historyService;
    }
    ...
    private static void drawHighLigh1(MyDefaultProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo) {
    ...
    protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,
        ...
        if (drawInstruction != null) {
            ...
            if (highLightedActivities.contains(flowNode.getId())) {
                // 获取当前节点并且未完成
                HistoricActivityInstance historicActivityInstance = myDefaultProcessDiagramGenerator.historyService.createHistoricActivityInstanceQuery()
                        .processInstanceId(highLightedActivities.get(0)).unfinished().activityId(flowNode.getId()).singleResult();
                if (historicActivityInstance != null && !flowNode.getId().equals("endenv")) {
                    //更改源码没有走的红色表示
                    drawHighLigh1((MyDefaultProcessDiagramCanvas) processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()));
                } else {
