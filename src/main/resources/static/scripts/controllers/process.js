...
angular.module('flowableModeler')
  .controller('ProcessCtrl', ['$rootScope', '$scope', '$translate', '$http', '$location', '$routeParams','$modal', '$popover', '$timeout', 'appResourceRoot', 'ResourceService',
    ...
    $scope.export = function() {
        $http({
            method: 'POST',
            url: FLOWABLE.APP_URL.getExportUrl(),
            params: {modelId: $routeParams.modelId}
        }).success(function(data, status, headers, config) {
            $scope.addAlertPromise($translate('PROCESS.export.success'), 'info');


        }).
        error(function(data, status, headers, config) {
            $scope.addAlertPromise($translate('PROCESS.export.error'), 'error');
        });
