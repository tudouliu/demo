...
flowableAdminApp.controller('BatchPartController', ['$scope', '$rootScope', '$http', '$timeout', '$location', '$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q, gridConstants) {
        ...
        $scope.loadBatchPart = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.webContextRoot + '/app/rest/admin/batch-parts/' + $routeParams.batchPartId
        ...
        $scope.loadProcessDefinitions = function () {
            if ($scope.batchPart.batchType == 'processMigration') {
                ...
                $http({
                    method: 'GET',
                    url: FlowableAdmin.webContextRoot + '/app/rest/admin/process-definitions/' + $scope.batchPart.searchKey
                }).success(function (data, status, headers, config) {
                ...
                $http({
                    method: 'GET',
                    url: FlowableAdmin.webContextRoot + '/app/rest/admin/process-definitions/' + $scope.batchPart.searchKey2
                }).success(function (data, status, headers, config) {
        ...
        $scope.loadBatchPartDocument = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.webContextRoot + '/app/rest/admin/batch-parts/' + $scope.batchPart.id + '/batch-part-document'
            }).success(function (data, status, headers, config) {
