...
flowableAdminApp.controller('BatchController', ['$scope', '$rootScope', '$http', '$timeout', '$location', '$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q, gridConstants) {
        ...
        $scope.loadBatch = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.webContextRoot + '/app/rest/admin/batches/' + $routeParams.batchId
        ...
        $scope.loadProcessDefinitions = function () {
            if ($scope.batch.batchType == 'processMigration') {
                ...
                $http({
                    method: 'GET',
                    url: FlowableAdmin.webContextRoot + '/app/rest/admin/process-definitions/' + $scope.batch.searchKey
                }).success(function (data, status, headers, config) {
                ...
                $http({
                    method: 'GET',
                    url: FlowableAdmin.webContextRoot + '/app/rest/admin/process-definitions/' + $scope.batch.searchKey2
                }).success(function (data, status, headers, config) {
        ...
        $scope.loadBatchDocument = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.webContextRoot + '/app/rest/admin/batches/' + $scope.batch.id + '/batch-document'
            }).success(function (data, status, headers, config) {
        ...
        $scope.loadSuccessParts = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.webContextRoot + '/app/rest/admin/batches/' + $scope.batch.id + '/batch-parts?status=success'
            }).success(function (data, status, headers, config) {
        ...
        $scope.loadFailParts = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.webContextRoot + '/app/rest/admin/batches/' + $scope.batch.id + '/batch-parts?status=fail'
            }).success(function (data, status, headers, config) {
...
flowableAdminApp.controller('DeleteBatchModalInstanceCtrl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'batch', function ($rootScope, $scope, $modalInstance, $http, batch) {
        ...
        $scope.ok = function () {
            ...
            $http({
                method: 'POST', url: FlowableAdmin.webContextRoot + '/app/rest/admin/batches/' + $scope.batch.id
